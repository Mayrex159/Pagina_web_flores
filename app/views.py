...
def carrito(request):
    carrito = Productos_carrito.objects.all().raw(
        "SELECT codigoCarrito, nombreProducto, CAST(COUNT(nombreProducto) AS INT) AS cantidad, CAST(sum(precioProducto) AS FLOAT) AS montoTotal, username as nomUsuario, suscripcion1 as codUsuario FROM db_producto_carrito INNER JOIN db_suscripcion WHERE username = 'admin'  GROUP BY nombreProducto")
    datos = {
...
def historial_compras(request):
    historia_compra = Productos_carrito.objects.all()
...
def productos(request):
    productos = Producto.objects.all().raw(
        "SELECT codigo, NOMBRE, MARCA, PRECIO, STOCK FROM db_producto")
    ...
    if request.method == 'POST':
        ...
        carrito.codigoProducto_id = request.POST.get('codigoProducto')
...
def eliminar_producto_carrito(request, codigoProducto_id):
    productoCarrito = get_object_or_404(
        Productos_carrito, codigoProducto_id=codigoProducto_id)
    productoCarrito.delete()
    producto = Producto.objects.all().raw(
        "SELECT codigo, NOMBRE, MARCA, PRECIO, STOCK as stock FROM db_producto WHERE codigo =" + codigoProducto_id)
    datos = {
        'producto': producto,
    }
    producto = Producto.objects.get(codigo=codigoProducto_id)
    producto.stock += 1
    producto.save()
...
def registro_usuarios(request):
    ...
    if request.method == 'POST':
        ...
        if formulario.is_valid():            
            formulario.save()
            usergroup = User.objects.get(id = formulario.instance.id)
